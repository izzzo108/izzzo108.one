/**
 * Car Road — 2.5D side-scroller sandbox
 */
(function () {
    const MODELS_BASE = 'assets/models/';
    const TEXTURES_BASE = 'assets/textures/';
    const CAR_X = -2.2;
    const GROUND_Y = -0.85;
    const ROAD_Y = GROUND_Y + 0.14;
    const ROAD_THICKNESS = 0.14;
    const ROAD_SURFACE_Y = ROAD_Y + ROAD_THICKNESS;
    const TILE_WIDTH = 28;
    const ROAD_DEPTH = 2.4;
    const GROUND_DEPTH = 72;
    const GROUND_TEX_TILE = 36;
    const BASE_SPEED = 5;
    const BOOST_MULT = 2.5;
    const RAMP_SPAWN_AHEAD = 24;
    const GRAVITY = -18;
    const JUMP_VY = 6.2;
    const CAR_RX = /^Car_/i;

    const prefersReducedMotion = matchMedia('(prefers-reduced-motion: reduce)').matches;

    let renderer, scene, camera;
    let car, carTemplate, rampTemplate;
    let carAnimations = [];
    let carMixer = null;
    let carGroundOffset = 0;
    let selectedCarFile = null;
    let carFiles = [];
    let carIndex = 0;
    let carSwapBusy = false;
    let modelCache = new Map();
    let roadTiles = [];
    let groundTiles = [];
    let roadMaterial = null;
    let roadTopMaterial = null;
    let roadSideMaterial = null;
    let groundMaterial = null;
    let roadUvScroll = 0;
    let groundUvScroll = 0;
    let ramps = [];
    let dustParticles = [];

    let gasHeld = false;
    let scrollSpeed = BASE_SPEED;
    let carJumpY = 0;
    let carVy = 0;
    let isJumping = false;
    let camShake = 0;
    let lastT = 0;
    let rampCooldown = 0;
    let rampRideHold = 0;
    const CAMERA_RADIUS_MIN = 4.8;
    const CAMERA_RADIUS_MAX = 13.5;
    const cameraOrbit = {
        yaw: -0.55,
        pitch: 0.2,
        radius: 7.8,
        dragging: false,
        pointerId: null,
        lastX: 0,
        lastY: 0
    };
    const orbitPointers = new Map();
    let pinchStartDist = 0;
    let pinchStartRadius = cameraOrbit.radius;

    function clampCameraRadius(r) {
        return Math.max(CAMERA_RADIUS_MIN, Math.min(CAMERA_RADIUS_MAX, r));
    }

    function orbitPointerDistance() {
        const pts = [...orbitPointers.values()];
        if (pts.length < 2) return 0;
        const dx = pts[0].x - pts[1].x;
        const dy = pts[0].y - pts[1].y;
        return Math.hypot(dx, dy);
    }

    function syncPinchBaseline() {
        pinchStartDist = orbitPointerDistance();
        pinchStartRadius = cameraOrbit.radius;
    }

    const canvas = document.getElementById('game-canvas');
    const gameLoading = document.getElementById('game-loading');
    const gameUi = document.getElementById('game-ui');
    const btnGas = document.getElementById('btnGas');
    const btnRamp = document.getElementById('btnRamp');
    const btnNextCar = document.getElementById('btnNextCar');

    function assetUrl(path) {
        return new URL(path, document.baseURI).href;
    }

    function waitForThree() {
        return window.GltfHelpers?.waitForThree() ?? Promise.resolve(false);
    }

    function listPlayableModels(files) {
        const list = files
            .filter((f) => /\.glb$/i.test(f))
            .filter((f) => CAR_RX.test(f.split('/').pop() || f));
        const byName = (a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' });
        list.sort(byName);
        return list;
    }

    async function getModelScene(file) {
        const H = window.GltfHelpers;
        if (modelCache.has(file)) return modelCache.get(file);
        const gltf = await H.loadGlb(assetUrl(MODELS_BASE + file));
        const data = { scene: gltf.scene, animations: gltf.animations || [] };
        modelCache.set(file, data);
        return data;
    }

    function createSurfaceMaterial(color, roughness = 0.92) {
        const THREE = window.THREE;
        return new THREE.MeshStandardMaterial({
            color,
            roughness,
            metalness: 0,
            side: THREE.DoubleSide
        });
    }

    function loadTexture(url) {
        const THREE = window.THREE;
        return new Promise((resolve, reject) => {
            new THREE.TextureLoader().load(url, resolve, undefined, reject);
        });
    }

    function setupRepeatingTexture(tex, repeatX, repeatY, rotate90 = true) {
        const THREE = window.THREE;
        tex.wrapS = THREE.RepeatWrapping;
        tex.wrapT = THREE.RepeatWrapping;
        tex.repeat.set(repeatX, repeatY);
        tex.center.set(0.5, 0.5);
        tex.rotation = rotate90 ? Math.PI / 2 : 0;
        if ('colorSpace' in tex && THREE.SRGBColorSpace) {
            tex.colorSpace = THREE.SRGBColorSpace;
        }
        return tex;
    }

    async function loadWorldMaterials() {
        const THREE = window.THREE;
        roadMaterial = createSurfaceMaterial(0x3a3a3a, 0.88);
        roadSideMaterial = new THREE.MeshStandardMaterial({
            color: 0x2a2a2a,
            roughness: 0.96,
            metalness: 0
        });
        groundMaterial = createSurfaceMaterial(0x5c7a42);

        try {
            const roadTex = setupRepeatingTexture(
                await loadTexture(assetUrl(TEXTURES_BASE + 'road.png')),
                1,
                1,
                true
            );
            roadTopMaterial = new THREE.MeshStandardMaterial({
                map: roadTex,
                roughness: 0.88,
                metalness: 0,
                side: THREE.DoubleSide
            });
            roadMaterial = roadTopMaterial;
        } catch (e) {
            console.warn('road.png not found — solid color road', e);
            roadTopMaterial = roadMaterial;
        }

        try {
            const groundTex = setupRepeatingTexture(
                await loadTexture(assetUrl(TEXTURES_BASE + 'ground.png')),
                1,
                1,
                true
            );
            groundMaterial = new THREE.MeshStandardMaterial({
                map: groundTex,
                roughness: 0.92,
                metalness: 0,
                side: THREE.DoubleSide
            });
        } catch (e) {
            console.warn('ground.png not found — solid color ground', e);
        }
    }

    function makeGroundTile(x) {
        const THREE = window.THREE;
        const geo = new THREE.PlaneGeometry(TILE_WIDTH, GROUND_DEPTH);
        const uv = geo.attributes.uv;
        const depthTiles = GROUND_DEPTH / GROUND_TEX_TILE;
        for (let i = 0; i < uv.count; i++) {
            uv.setY(i, uv.getY(i) * depthTiles);
        }
        const mesh = new THREE.Mesh(geo, groundMaterial);
        mesh.rotation.x = -Math.PI / 2;
        // Земля чуть ниже дороги (ближе по высоте, чем раньше)
        mesh.position.set(x, GROUND_Y - 0.02, 0);
        mesh.receiveShadow = false;
        scene.add(mesh);
        return { mesh, width: TILE_WIDTH };
    }

    function makeRoadTile(x) {
        const THREE = window.THREE;
        const mats = [
            roadSideMaterial || roadMaterial,
            roadSideMaterial || roadMaterial,
            roadTopMaterial || roadMaterial,
            roadSideMaterial || roadMaterial,
            roadSideMaterial || roadMaterial,
            roadSideMaterial || roadMaterial
        ];
        const mesh = new THREE.Mesh(
            // Дорога — отдельный 3D-объект (платформа)
            new THREE.BoxGeometry(TILE_WIDTH, ROAD_THICKNESS, ROAD_DEPTH),
            mats
        );
        mesh.position.set(x, ROAD_Y + ROAD_THICKNESS * 0.5, 0);
        mesh.receiveShadow = false;
        scene.add(mesh);
        return { mesh, width: TILE_WIDTH };
    }

    function initWorld() {
        const THREE = window.THREE;

        // Поздний закат: градиент (сфера неба, без тумана)
        const SKY_RADIUS = 220;
        const skyGeo = new THREE.SphereGeometry(SKY_RADIUS, 48, 24);
        const skyMat = new THREE.ShaderMaterial({
            side: THREE.BackSide,
            fog: false,
            depthWrite: false,
            uniforms: {
                uTop: { value: new THREE.Color(0x3d2f78) },
                uMid: { value: new THREE.Color(0xe87840) },
                uBot: { value: new THREE.Color(0xffb347) }
            },
            vertexShader: `
                varying vec3 vWorldPos;
                void main() {
                    vec4 wp = modelMatrix * vec4(position, 1.0);
                    vWorldPos = wp.xyz;
                    gl_Position = projectionMatrix * viewMatrix * wp;
                }
            `,
            fragmentShader: `
                uniform vec3 uTop;
                uniform vec3 uMid;
                uniform vec3 uBot;
                varying vec3 vWorldPos;
                void main() {
                    vec3 dir = normalize(vWorldPos);
                    float t = clamp(dir.y * 0.5 + 0.5, 0.0, 1.0);
                    float horizon = smoothstep(0.08, 0.38, t);
                    float zenith = smoothstep(0.45, 0.92, t);
                    vec3 col = mix(uBot, uMid, horizon);
                    col = mix(col, uTop, zenith);
                    col += vec3(0.12, 0.04, 0.02) * (1.0 - abs(t - 0.32) * 3.0);
                    gl_FragColor = vec4(col, 1.0);
                }
            `
        });
        const sky = new THREE.Mesh(skyGeo, skyMat);
        sky.renderOrder = -1000;
        scene.add(sky);

        scene.add(new THREE.HemisphereLight(0xffc89b, 0x8b5a3c, 0.5));
        scene.add(new THREE.AmbientLight(0xffffff, 0.42));
        const sun = new THREE.DirectionalLight(0xfff5e0, 0.85);
        sun.position.set(5, 10, 8);
        scene.add(sun);

        for (let i = 0; i < 3; i++) {
            // Центрируем дорогу под машиной (убирает визуальный "уезд в бок")
            const x = CAR_X + (i - 1) * TILE_WIDTH;
            groundTiles.push(makeGroundTile(x));
            roadTiles.push(makeRoadTile(x));
        }
    }

    function scrollTiles(tiles, move) {
        for (const tile of tiles) {
            tile.mesh.position.x -= move;
        }
        for (const tile of tiles) {
            if (tile.mesh.position.x <= -TILE_WIDTH * 1.5) {
                let maxX = -Infinity;
                for (const t of tiles) maxX = Math.max(maxX, t.mesh.position.x);
                tile.mesh.position.x = maxX + tile.width;
            }
        }
    }

    async function loadCar(carFile) {
        const H = window.GltfHelpers;
        if (!carFile) {
            const files = await H.loadModelsManifest(assetUrl(MODELS_BASE));
            const models = listPlayableModels(files);
            carFile = selectedCarFile || models[0] || H.pickModelFile(files, 'car');
        }
        if (!carFile) {
            console.warn('No car .glb in assets/models/');
            return buildPlaceholderCar();
        }
        try {
            if (car) {
                scene.remove(car);
                disposeObject(car);
                car = null;
            }
            const loaded = await getModelScene(carFile);
            carTemplate = loaded.scene;
            carAnimations = loaded.animations || [];
            H.prepareCarScene(carTemplate, { groundY: 0 });
            car = carTemplate.clone(true);
            car.rotation.y = Math.PI;
            scene.add(car);
            if (carMixer) {
                carMixer.stopAllAction();
                carMixer = null;
            }
            if (carAnimations.length) {
                const THREE = window.THREE;
                carMixer = new THREE.AnimationMixer(car);
                carAnimations.forEach((clip) => {
                    const action = carMixer.clipAction(clip);
                    action.reset();
                    action.setEffectiveTimeScale(1);
                    action.setEffectiveWeight(1);
                    action.play();
                });
            }
            updateCarPosition();
            car.updateMatrixWorld(true);
            const box = new window.THREE.Box3().setFromObject(car);
            carGroundOffset = car.position.y - box.min.y;
            return true;
        } catch (e) {
            console.warn('Car load failed', e);
            return buildPlaceholderCar();
        }
    }

    async function initCarList() {
        const H = window.GltfHelpers;
        const files = await H.loadModelsManifest(assetUrl(MODELS_BASE));
        carFiles = listPlayableModels(files);
        if (!carFiles.length && selectedCarFile) carFiles = [selectedCarFile];
        carIndex = carFiles.indexOf(selectedCarFile);
        if (carIndex < 0) carIndex = 0;
    }

    async function nextCar() {
        if (!carFiles.length) return;
        if (carSwapBusy) return;
        carSwapBusy = true;
        try {
            carIndex = (carIndex + 1) % carFiles.length;
            selectedCarFile = carFiles[carIndex];
            await loadCar(selectedCarFile);
            updateCarPosition();
        } finally {
            carSwapBusy = false;
        }
    }

    function buildPlaceholderCar() {
        const THREE = window.THREE;
        const body = new THREE.Mesh(
            new THREE.BoxGeometry(1.4, 0.5, 0.7),
            new THREE.MeshStandardMaterial({ color: 0xfbbf24 })
        );
        car = new THREE.Group();
        car.add(body);
        carAnimations = [];
        carMixer = null;
        car.rotation.y = Math.PI;
        scene.add(car);
        updateCarPosition();
        car.updateMatrixWorld(true);
        const box = new THREE.Box3().setFromObject(car);
        carGroundOffset = car.position.y - box.min.y;
        return true;
    }

    async function loadRampTemplate() {
        const H = window.GltfHelpers;
        const files = await H.loadModelsManifest(assetUrl(MODELS_BASE));
        const rampFile = H.pickModelFile(files, 'ramp');
        if (rampFile) {
            try {
                const gltf = await H.loadGlb(assetUrl(MODELS_BASE + rampFile));
                rampTemplate = gltf.scene;
                H.prepareRampScene(rampTemplate, { flipY: true, groundY: 0 });
                return;
            } catch (e) {
                console.warn('Ramp glb failed', e);
            }
        }
        const THREE = window.THREE;
        rampTemplate = new THREE.Group();
        const ramp = new THREE.Mesh(
            new THREE.BoxGeometry(2, 0.4, 1.2),
            new THREE.MeshStandardMaterial({ color: 0xa855f7 })
        );
        ramp.position.y = 0.2;
        ramp.rotation.z = 0.35;
        rampTemplate.add(ramp);
    }

    function updateCarPosition() {
        if (!car) return;
        car.position.x = CAR_X;
        car.position.y = ROAD_SURFACE_Y + carJumpY;
        car.position.z = 0;
        if (isJumping || carJumpY > 0.01) {
            car.rotation.z = Math.sin(performance.now() * 0.012) * 0.15 - 0.1;
        } else {
            car.rotation.z *= 0.92;
        }
    }

    function spawnRamp() {
        if (rampCooldown > 0 || ramps.length >= 2) return;
        const THREE = window.THREE;
        let ramp;
        if (rampTemplate) {
            ramp = rampTemplate.clone(true);
        } else {
            ramp = new THREE.Mesh(
                new THREE.BoxGeometry(2, 0.5, 1),
                new THREE.MeshStandardMaterial({ color: 0xc084fc })
            );
        }
        // Трамплин стоит на дороге
        ramp.position.set(CAR_X + RAMP_SPAWN_AHEAD, ROAD_SURFACE_Y, 0);
        scene.add(ramp);
        ramp.updateMatrixWorld(true);
        const profile = window.RampCollision?.computeRampProfile(ramp) || null;
        ramps.push({ mesh: ramp, triggered: false, profile, riding: false });
        rampCooldown = 0.4;
    }

    const DUST_MAX = 200;

    function spawnDust(mode = 'boost') {
        if (!car || dustParticles.length >= DUST_MAX) return;
        const THREE = window.THREE;
        const isIdle = mode === 'idle';
        const size = isIdle
            ? 0.06 + Math.random() * 0.06
            : 0.09 + Math.random() * 0.11;
        const shade = 0x5a5f66 + Math.floor(Math.random() * 0x303030);
        const p = new THREE.Mesh(
            new THREE.SphereGeometry(size, 5, 5),
            new THREE.MeshBasicMaterial({
                color: shade,
                transparent: true,
                opacity: isIdle ? 0.35 + Math.random() * 0.15 : 0.55 + Math.random() * 0.25
            })
        );
        const wheelZ = isIdle
            ? (Math.random() - 0.5) * 0.12
            : (Math.random() < 0.5 ? -0.45 : 0.45);
        p.position.set(
            CAR_X - 0.08 - Math.random() * (isIdle ? 0.22 : 0.45),
            ROAD_SURFACE_Y + 0.02 + Math.random() * (isIdle ? 0.02 : 0.04),
            wheelZ + (Math.random() - 0.5) * 0.12
        );
        scene.add(p);
        const life = isIdle
            ? 0.45 + Math.random() * 0.35
            : 0.75 + Math.random() * 0.55;
        dustParticles.push({
            mesh: p,
            life,
            maxLife: life,
            vx: isIdle ? -2.2 - Math.random() * 1.1 : -4.5 - Math.random() * 3,
            vy: isIdle ? 0.18 + Math.random() * 0.45 : 0.35 + Math.random() * 1.1,
            vz: isIdle ? (Math.random() - 0.5) * 0.45 : (Math.random() - 0.5) * 1.4
        });
    }

    function scrollWorld(dt, speed) {
        const move = speed * dt;

        scrollTiles(groundTiles, move);
        scrollTiles(roadTiles, move);
        if (groundMaterial?.map) {
            groundUvScroll -= move / TILE_WIDTH * 1.15;
            groundMaterial.map.offset.y = groundUvScroll % 1;
        }
        if (roadMaterial?.map) {
            roadUvScroll -= move / TILE_WIDTH * 1.15;
            roadMaterial.map.offset.y = roadUvScroll % 1;
        }

        const RC = window.RampCollision;
        const carBottom = RC?.getCarBottomY(car) ?? (ROAD_SURFACE_Y + carJumpY);

        for (let i = ramps.length - 1; i >= 0; i--) {
            const r = ramps[i];
            r.mesh.position.x -= move;
            if (r.profile && RC) {
                const contact = RC.checkRampContact(r, car, carBottom);
                if (contact.hit && contact.surfaceY !== null) {
                    const targetJumpY = contact.surfaceY - ROAD_SURFACE_Y + carGroundOffset;
                    if (!isJumping || carJumpY < targetJumpY + 0.12) {
                        carJumpY = Math.max(carJumpY, targetJumpY);
                        carVy = Math.max(carVy, 0);
                        r.riding = true;
                        rampRideHold = 0.18;
                    }
                    if (!r.triggered && contact.onLip) {
                        r.triggered = true;
                        isJumping = true;
                        carVy = JUMP_VY;
                        r.riding = false;
                    }
                } else {
                    r.riding = false;
                }
            }
            if (r.mesh.position.x < -12) {
                scene.remove(r.mesh);
                disposeObject(r.mesh);
                ramps.splice(i, 1);
            }
        }
    }

    function disposeObject(root) {
        root.traverse((child) => {
            if (child.geometry) child.geometry.dispose();
            if (child.material) {
                const mats = Array.isArray(child.material) ? child.material : [child.material];
                mats.forEach((m) => m.dispose());
            }
        });
    }

    function updateJump(dt) {
        if (rampRideHold > 0) rampRideHold -= dt;
        if (ramps.some((r) => r.riding) || rampRideHold > 0) {
            carVy = 0;
            return;
        }
        if (!isJumping && carJumpY <= 0) {
            carJumpY = 0;
            carVy = 0;
            return;
        }
        carVy += GRAVITY * dt;
        carJumpY += carVy * dt;
        if (carJumpY <= 0) {
            carJumpY = 0;
            carVy = 0;
            isJumping = false;
            camShake = 0.15;
        }
    }

    function updateDust(dt) {
        for (let i = dustParticles.length - 1; i >= 0; i--) {
            const d = dustParticles[i];
            d.life -= dt;
            d.mesh.position.x += d.vx * dt;
            d.mesh.position.y += (d.vy || 0) * dt;
            d.mesh.position.z += (d.vz || 0) * dt;
            const maxLife = d.maxLife || 0.75;
            const fade = d.life / maxLife;
            d.mesh.material.opacity = fade * 0.85;
            d.mesh.scale.setScalar(0.7 + (1 - fade) * 2.2);
            if (d.life <= 0) {
                scene.remove(d.mesh);
                d.mesh.geometry.dispose();
                d.mesh.material.dispose();
                dustParticles.splice(i, 1);
            }
        }
        if (gasHeld) {
            const burst = 7 + Math.floor(Math.random() * 5);
            for (let n = 0; n < burst; n++) spawnDust('boost');
        } else if (scrollSpeed >= BASE_SPEED * 0.95) {
            const idleBurst = 1 + (Math.random() < 0.45 ? 1 : 0);
            for (let n = 0; n < idleBurst; n++) spawnDust('idle');
        }
    }

    function loop(t) {
        if (!lastT) lastT = t;
        const dt = Math.min(0.05, (t - lastT) / 1000);
        lastT = t;
        if (carMixer) carMixer.update(dt);

        if (rampCooldown > 0) rampCooldown -= dt;

        scrollSpeed = gasHeld ? BASE_SPEED * BOOST_MULT : BASE_SPEED;
        if (gasHeld) camShake = Math.min(0.08, camShake + dt * 2);

        scrollWorld(dt, scrollSpeed);
        updateJump(dt);
        updateCarPosition();
        updateDust(dt);

        if (camera) {
            const shakeX = camShake > 0 ? (Math.random() - 0.5) * camShake : 0;
            const shakeY = camShake > 0 ? (Math.random() - 0.5) * camShake * 0.5 : 0;
            const lookX = CAR_X;
            const lookY = ROAD_SURFACE_Y + carJumpY + 0.45;
            const lookZ = 0;
            const cosPitch = Math.cos(cameraOrbit.pitch);
            const sinPitch = Math.sin(cameraOrbit.pitch);
            const cosYaw = Math.cos(cameraOrbit.yaw);
            const sinYaw = Math.sin(cameraOrbit.yaw);
            camera.position.x = lookX + cameraOrbit.radius * cosPitch * cosYaw + shakeX;
            camera.position.y = lookY + cameraOrbit.radius * sinPitch + shakeY;
            camera.position.y = Math.max(ROAD_SURFACE_Y + 0.75, camera.position.y);
            camera.position.z = lookZ + cameraOrbit.radius * cosPitch * sinYaw;
            camera.lookAt(lookX, lookY, lookZ);
            camShake *= 0.9;
        }

        renderer.render(scene, camera);
        requestAnimationFrame(loop);
    }

    function bindControls() {
        const setGas = (on) => {
            gasHeld = on;
            btnGas.classList.toggle('is-active', on);
        };
        btnGas.addEventListener('pointerdown', (e) => { e.preventDefault(); setGas(true); });
        btnGas.addEventListener('pointerup', () => setGas(false));
        btnGas.addEventListener('pointerleave', () => setGas(false));
        btnGas.addEventListener('pointercancel', () => setGas(false));

        btnRamp.addEventListener('click', (e) => {
            e.preventDefault();
            spawnRamp();
        });

        if (btnNextCar) {
            btnNextCar.addEventListener('click', (e) => {
                e.preventDefault();
                nextCar();
            });
        }

        const canStartDrag = (target) => {
            if (!target) return true;
            return !target.closest('#car-select, .controls, .hub-back, .btn');
        };

        window.addEventListener('pointerdown', (e) => {
            if (!canStartDrag(e.target)) return;
            orbitPointers.set(e.pointerId, { x: e.clientX, y: e.clientY });
            if (orbitPointers.size === 1) {
                cameraOrbit.dragging = true;
                cameraOrbit.pointerId = e.pointerId;
                cameraOrbit.lastX = e.clientX;
                cameraOrbit.lastY = e.clientY;
            } else if (orbitPointers.size >= 2) {
                cameraOrbit.dragging = false;
                cameraOrbit.pointerId = null;
                syncPinchBaseline();
            }
        });

        window.addEventListener('pointermove', (e) => {
            if (!orbitPointers.has(e.pointerId)) return;
            orbitPointers.set(e.pointerId, { x: e.clientX, y: e.clientY });

            if (orbitPointers.size >= 2) {
                const dist = orbitPointerDistance();
                if (pinchStartDist > 1) {
                    const scale = dist / pinchStartDist;
                    cameraOrbit.radius = clampCameraRadius(pinchStartRadius / scale);
                }
                return;
            }

            if (!cameraOrbit.dragging || cameraOrbit.pointerId !== e.pointerId) return;
            const dx = e.clientX - cameraOrbit.lastX;
            const dy = e.clientY - cameraOrbit.lastY;
            cameraOrbit.lastX = e.clientX;
            cameraOrbit.lastY = e.clientY;
            cameraOrbit.yaw -= dx * 0.006;
            cameraOrbit.pitch = Math.max(0.03, Math.min(0.78, cameraOrbit.pitch - dy * 0.004));
        });

        const stopOrbitPointer = (e) => {
            orbitPointers.delete(e.pointerId);
            if (cameraOrbit.pointerId === e.pointerId) {
                cameraOrbit.dragging = false;
                cameraOrbit.pointerId = null;
            }
            if (orbitPointers.size >= 2) {
                syncPinchBaseline();
            } else {
                pinchStartDist = 0;
            }
        };
        window.addEventListener('pointerup', stopOrbitPointer);
        window.addEventListener('pointercancel', stopOrbitPointer);

        const onWheelZoom = (e) => {
            if (canvas?.classList.contains('is-hidden')) return;
            if (e.target?.closest?.('#car-select, .controls, .hub-back, .btn')) return;
            e.preventDefault();
            cameraOrbit.radius = clampCameraRadius(cameraOrbit.radius + e.deltaY * 0.004);
        };
        window.addEventListener('wheel', onWheelZoom, { passive: false });
    }

    function waitForCarSelection() {
        if (window.CarRoad?.selectedCarFile) {
            selectedCarFile = window.CarRoad.selectedCarFile;
            return Promise.resolve(selectedCarFile);
        }
        return new Promise((resolve) => {
            window.addEventListener('car-selected', (e) => {
                selectedCarFile = e.detail.file;
                resolve(selectedCarFile);
            }, { once: true });
        });
    }

    async function init() {
        if (prefersReducedMotion) {
            const hint = document.querySelector('.hint-bar');
            if (hint) hint.textContent = 'Анимация отключена в системе';
            return;
        }
        const ready = await waitForThree();
        if (!ready) {
            console.error('Three.js not loaded');
            return;
        }

        const THREE = window.THREE;
        renderer = new THREE.WebGLRenderer({ canvas, antialias: true, alpha: false });
        renderer.setPixelRatio(Math.min(devicePixelRatio || 1, 2));
        scene = new THREE.Scene();
        scene.background = new THREE.Color(0x3d2f78);
        scene.fog = new THREE.Fog(0xb86a50, 28, 70);

        const aspect = innerWidth / innerHeight;
        camera = new THREE.PerspectiveCamera(28, aspect, 0.1, 80);
        camera.position.set(4, 2.2, 11);

        function resize() {
            const w = innerWidth, h = innerHeight;
            renderer.setSize(w, h, false);
            camera.aspect = w / h;
            camera.updateProjectionMatrix();
        }
        resize();
        window.addEventListener('resize', resize);

        await loadWorldMaterials();
        initWorld();
        await loadRampTemplate();
        await initCarList();
        await loadCar(selectedCarFile);
        updateCarPosition();
        bindControls();
        requestAnimationFrame(loop);
    }

    function showGameLoading() {
        if (gameLoading) {
            gameLoading.classList.remove('is-hidden');
            gameLoading.setAttribute('aria-busy', 'true');
        }
    }

    function hideGameLoading() {
        if (gameLoading) {
            gameLoading.classList.add('is-hidden');
            gameLoading.setAttribute('aria-busy', 'false');
        }
    }

    function revealGame() {
        if (canvas) canvas.classList.remove('is-hidden');
        if (gameUi) gameUi.classList.remove('is-hidden');
    }

    function lockGameUntilCarPick() {
        const pick = document.getElementById('car-select');
        if (pick) {
            pick.classList.remove('is-hidden');
            pick.setAttribute('aria-hidden', 'false');
        }
        hideGameLoading();
        if (canvas) canvas.classList.add('is-hidden');
        if (gameUi) gameUi.classList.add('is-hidden');
    }

    async function bootstrap() {
        lockGameUntilCarPick();
        await waitForCarSelection();
        showGameLoading();
        try {
            await init();
            revealGame();
        } catch (e) {
            console.error('Game init failed', e);
            const text = gameLoading?.querySelector('.game-loading__text');
            if (text) text.textContent = 'Ошибка загрузки';
        } finally {
            hideGameLoading();
        }
    }

    window.CarRoad = window.CarRoad || {};
    bootstrap();
})();
